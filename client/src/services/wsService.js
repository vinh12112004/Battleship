  ...
  serializeMessage(type, payload, token = "") {
    ...
    if (type === MSG_TYPES.REGISTER || type === MSG_TYPES.LOGIN) {
      ...
      const typeBeforePayload = Array.from(uint8.slice(0, 4))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join(" ");
      console.log(`[WS] Type field BEFORE payload write: ${typeBeforePayload}`);
      ...
      const typeAfterGameId = Array.from(uint8.slice(0, 4))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join(" ");
      console.log(`[WS] Type field AFTER game_id write: ${typeAfterGameId}`);
      ...
      const typeAfterMessage = Array.from(uint8.slice(0, 4))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join(" ");
      console.log(`[WS] Type field AFTER message write: ${typeAfterMessage}`);
      ...
      const sentGameId = new TextDecoder()
        .decode(uint8.slice(this.OFFSET_PAYLOAD, this.OFFSET_PAYLOAD + 64))
        .replace(/\0/g, "");
