...
class WebSocketService {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ws.onclose = (event) => {
                ...
                if (
                    ...
                    console.error(
                        "[WS] ❌ Max reconnect attempts reached. Giving up."
                    );
    ...
    serializeMessage(type, payload, token = "") {
        ...
        if (token) {
            ...
            uint8.set(
                tokenBytes.slice(0, this.MAX_JWT_LEN - 1),
                this.OFFSET_TOKEN
            );
        }
        ...
        if (type === MSG_TYPES.REGISTER || type === MSG_TYPES.LOGIN) {
            ...
            if (payload.board_state && Array.isArray(payload.board_state)) {
                // Chuyển mảng thường thành Uint8Array để set vào buffer
                const boardBytes = new Uint8Array(payload.board_state);
                // Lưu ý: Đảm bảo độ dài là 100, nếu ngắn hơn thì set, dài hơn thì cắt
                uint8.set(boardBytes.slice(0, 100), this.OFFSET_PAYLOAD);
            }

            // 2. Ghi game_id (bắt đầu ngay sau board_state, tức là + 100 bytes)
            if (payload.game_id) {
                const gameIdBytes = new TextEncoder().encode(payload.game_id);
                uint8.set(
                    gameIdBytes.slice(0, 64), // Cắt tối đa 64 ký tự (chừa 1 byte null)
                    this.OFFSET_PAYLOAD + 100 // 100 là kích thước của board_state (10x10)
                );
            }
        }
