...
export default function GamePage() {
  ...
  // ✅ Listen for TURN_WARNING
  useEffect(() => {
    const handleTurnWarning = (payload) => {
      console.log("[GamePage] ⏰ Turn warning:", payload);

      // ✅ Chỉ hiện toast, không update state
      toast.warning(
        `⏰ WARNING: ${payload.seconds_remaining} SECONDS REMAINING!`,
        {
          autoClose: 3000,
          style: {
            background: "#7f1d1d",
    ...
    wsService.onMessage(MSG_TYPES.TURN_WARNING, handleTurnWarning);

    return () => {
      wsService.offMessage(MSG_TYPES.TURN_WARNING, handleTurnWarning);
    };
  }, []);

  // ✅ Listen for GAME_TIMEOUT
  useEffect(() => {
    const handleGameTimeout = (payload) => {
      console.log("[GamePage] ⏰ Game timeout:", payload);
      ...
      console.log("[GamePage] Checking winner:", {
        myUsername: myUsername,
        winnerUsername: payload.winner_id, // Server should send username
        loserUsername: payload.loser_id,
      });

      // ✅ So sánh USERNAME (không phải ID)
      const isWinner = payload.winner_id === myUsername;

      console.log("[GamePage] 🔍 Am I winner?", isWinner);

      if (isWinner) {
        toast.success("🏆 VICTORY! Opponent ran out of time!", {
          autoClose: 5000,
          style: {
            background: "#065f46",
            color: "#d1fae5",
    ...
    wsService.onMessage(MSG_TYPES.GAME_TIMEOUT, handleGameTimeout);

    return () => {
      wsService.offMessage(MSG_TYPES.GAME_TIMEOUT, handleGameTimeout);
    };
  }, [navigate]);
